...
`).map(t=>`${e}${t}`).join(`
`)}var Ai=class s{constructor(o){this.id=o.versionId,this.name=o.name,this.prompt=o.prompt,this.commit=o.commit,this.type=o.type,this.metadata=o.metadata,this.changeDescription=o.changeDescription,this.tags=o.tags,this.createdAt=o.createdAt,this.createdBy=o.createdBy;}format(o){return me(this.prompt,o,this.type)}getVersionAge(){return this.createdAt?formatDistanceToNow(new Date(this.createdAt),{addSuffix:true}):"Unknown"}getVersionInfo(){let o=[`[${this.commit}]`];if(this.createdAt){let e=new Date(this.createdAt);o.push(e.toISOString().split("T")[0]);}return this.createdBy&&o.push(`by ${this.createdBy}`),this.changeDescription&&o.push(`- ${this.changeDescription}`),o.join(" ")}compareTo(o){let e=`Current version [${this.commit}]`,t=`Other version [${o.commit}]`,i=this.prompt,a=o.prompt;this.isChatPrompt(this.prompt)&&(i=this.formatChatPromptString(this.prompt)),this.isChatPrompt(o.prompt)&&(a=this.formatChatPromptString(o.prompt));let n=diffStringsUnified(a,i,{aAnnotation:t,bAnnotation:e,includeChangeCounts:true,contextLines:3,expand:false});return j.info(`
...
`)}function Qg(s,o,e,t){return `
    ...
    `}function Yg(s,o,e){return `
        ...
        ***
        ...
        ${o}
        ***
    `}function Xg(s,o){try{let e=Oe(s),t=e.answer_relevance_score;if(t==null)throw new Error(`Answer relevance score is required but got ${t}`);let i=Number(t);if(isNaN(i)||i<0||i>1)throw new Error(`Answer relevance score must be between 0.0 and 1.0, got ${i}`);return {name:o,value:i,reason:String(e.reason||"")}}catch(e){let t=e instanceof Error?e.message:String(e);throw j.error(`Failed to parse model output: ${t}`),new _e("Failed to calculate answer relevance score. The model output could not be parsed.",e instanceof Error?e:void 0)}}var Nw=z$1.object({input:z$1.string(),output:z$1.string(),context:z$1.array(z$1.string()).optional()}),Mw=z$1.object({answer_relevance_score:z$1.number(),reason:z$1.string()}),ts=class extends ve{constructor(e){var i,a,n,p;let t=(i=e==null?void 0:e.name)!=null?i:"answer_relevance_metric";super(t,{model:e==null?void 0:e.model,trackMetric:e==null?void 0:e.trackMetric,temperature:e==null?void 0:e.temperature,seed:e==null?void 0:e.seed,maxTokens:e==null?void 0:e.maxTokens,modelSettings:e==null?void 0:e.modelSettings});this.validationSchema=Nw;this.fewShotExamplesWithContext=(a=e==null?void 0:e.fewShotExamples)!=null?a:Kg,this.fewShotExamplesNoContext=(n=e==null?void 0:e.fewShotExamplesNoContext)!=null?n:$g,this.requireContext=(p=e==null?void 0:e.requireContext)!=null?p:true;}async score(e){let{input:t,output:i,context:a}=e;if(!a&&this.requireContext)throw new _e(`${this.name} requires context by default. If you want to allow execution in no-context mode, enable it via 'new AnswerRelevance({ requireContext: false })'`);let n=a?Qg(t,i,a,this.fewShotExamplesWithContext):Yg(t,i,this.fewShotExamplesNoContext),p=this.buildModelOptions(),l=await this.model.generateString(n,Mw,p);return Xg(l,this.name)}};export{ts as AnswerRelevance,ve as BaseLLMJudgeMetric,we as BaseMetric,Dg as Contains,Ig as ExactMatch,rs as Hallucination,Ng as IsJson,Ze as ModelConfigurationError,Ci as ModelError,qr as ModelGenerationError,qn as Moderation,zr as Opik,kr as OpikBaseModel,Ye as OpikQueryLanguage,Ce as OpikSpanType,$e as Prompt,xe as PromptType,Ug as RegexMatch,es as Usefulness,et as VercelAIChatModel,uf as createModel,jg as createModelFromInstance,df as detectProvider,ny as disableLogger,Sg as evaluate,Tge as evaluatePrompt,Yx as flushAll,ne as generateId,Gx as getTrackContext,j as logger,Ii as resolveModel,Pf as setLoggerLevel,Xe as track};